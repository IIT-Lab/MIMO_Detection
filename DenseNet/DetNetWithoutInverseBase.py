...
net_type = 1  # 0-普通网络 1-残差网络 2-密集网络
...
def strengthen_network(net, type, alpha, block_size):
    if type == 1:
        net[-1] = (1 - alpha) * net[-1] + alpha * net[-2]
        return net
    elif type == 2:
        net_len = len(net)
        net_mod_index = net_len % block_size
        alpha_per = alpha / (net_len - 1)
        net[-1] = (1 - alpha) * net[-1]
        for i in range(-net_mod_index, -1):
            net[-1] += alpha_per * net[i]
...
for i in range(1, L):
    ...
    S = strengthen_network(S, net_type, res_alpha, block_size)
    # S[i] = (1 - res_alpha) * S[i] + res_alpha * S[i - 1]
    V.append(affine_layer(ZZ, hl_size, v_size, 'aff' + str(i)))
    V = strengthen_network(V, net_type, res_alpha, block_size)
