...
def pre_process_before_training(K, H, sigma_2, flag=1):
    if flag == 1:  # MMSE预处理
        temp = H.T.dot(H) + sigma_2 * np.eye(K)
        target_mat = np.linalg.pinv(temp).dot(H.T)
    elif flag == 2:  # QR分解
        target_mat, r = np.linalg.qr(H)
        target_mat = target_mat.T
    elif flag == 3:  # svd分解
...
def generate_data_iid_test(B, K, N, snr_low, snr_high):
    ...
    for i in range(B):
        ...
        H_[i, :, :] = H
        y_[i, :] = (H.dot(x_[i, :]) + w[i, :])
        # mmse或zf等效H  sigma相当于1
        x_estimate[i, :] = pre_process_before_training(K, H, 1, pre_mode).dot(y_[i, :])
        Hy_[i, :] = H.T.dot(y_[i, :])
        HH_[i, :, :] = H.T.dot(H_[i, :, :])
        SNR_[i] = SNR
    return Hy_, HH_, x_, np.sign(x_estimate)
...
X_est = tf.placeholder(tf.float32, shape=[None, K])
...
for i in range(train_iter):  # num of train iter
    batch_HY, batch_HH, batch_X, x_est = generate_data_train(train_batch_size, K, N, snr_low, snr_high)
    train_step.run(feed_dict={HY: batch_HY, HH: batch_HH, X: batch_X, X_est: x_est})
    if i % 100 == 0:
        batch_HY, batch_HH, batch_X, x_est = generate_data_iid_test(train_batch_size, K, N, snr_low, snr_high)
        results = sess.run([LOSS[L - 1], BER[L - 1]], {HY: batch_HY, HH: batch_HH, X: batch_X, X_est: x_est})
...
for j in range(num_snr):
    for jj in range(test_iter):
        ...
        print(jj)
        batch_HY, batch_HH, batch_X, x_est = generate_data_iid_test(test_batch_size, K, N, snr_list[j], snr_list[j])
        tic = tm.time()
        tmp_bers[:, jj] = np.array(sess.run(BER[L - 1], {HY: batch_HY, HH: batch_HH, X: batch_X, X_est: x_est}))
        toc = tm.time()
