...
for i in range(1, L):
    ...
    if i <= 2:
        Z = tf.concat([HY] + S + [temp1], 1)
        ZZ = relu_layer(Z, 2 * K + len(S) * K, hl_size + len(S) * K, 'relu' + str(i))
    else:
        Z = tf.concat([HY] + S[2:len(S)] + [temp1], 1)
        ZZ = relu_layer(Z, 2 * K + (len(S)-2) * K, hl_size + len(S) * K, 'relu' + str(i))
    S.append(sign_layer(ZZ, hl_size + len(S) * K, K, 'sign' + str(i)))
