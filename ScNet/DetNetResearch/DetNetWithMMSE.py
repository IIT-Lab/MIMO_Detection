...
"""
没有对矩阵进行求逆的DetNet,增加了对输入矩阵进行预处理的过程
"""
...
def generate_data_iid_test(B, K, N, snr_low, snr_high):
    ...
    for i in range(B):
        ...
        W = pre_process_before_training(K, H, 1, pre_mode)
        tmp_H = W.dot(H)
        Hy_[i, :] = (tmp_H).T.dot(W).dot(y_[i, :])
        HH_[i, :, :] = tmp_H.T.dot(tmp_H)
        SNR_[i] = SNR
    return Hy_, HH_, x_
...
for i in range(train_iter):  # num of train iter
    batch_HY, batch_HH, batch_X = generate_data_train(train_batch_size, K, N, snr_low, snr_high)
    train_step.run(feed_dict={HY: batch_HY, HH: batch_HH, X: batch_X})
    if i % 100 == 0:
        batch_HY, batch_HH, batch_X = generate_data_iid_test(train_batch_size, K, N, snr_low, snr_high)
...
for j in range(num_snr):
    for jj in range(test_iter):
        ...
        print(jj)
        batch_HY, batch_HH, batch_X = generate_data_iid_test(test_batch_size, K, N, snr_list[j], snr_list[j])
        tic = tm.time()
