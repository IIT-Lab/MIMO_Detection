...
def piecewise_linear_soft_sign(x):
    t = tf.constant(0.1)
    y = -3 + tf.nn.relu(x + 2 + t) / (tf.abs(t) + 0.00001) - tf.nn.relu(x + 2 - t) / (tf.abs(t) + 0.00001) + tf.nn.relu(
        x + t) / (tf.abs(t) + 0.00001) - tf.nn.relu(x - t) / (tf.abs(t) + 0.00001) + tf.nn.relu(x - 2 + t) / (
...
weigth_matrix = np.zeros((3 * (2 * K), 16 * K))
for i in range(0, K):  # 遍历天线
    for j in range(0, 3):  # 遍历Hy、x、HHy
        for k in range(0, 16):  # 遍历16个符号
...
for i in range(1, L):
    ...
    S2.append(relu_layer(ZZ, hl_size, K * 16, 'sign' + str(i), weigth_matrix, 1))
...
saver.restore(sess, "./DetNet_HD_16QAM/ScNet16QAM_HD_model.ckpt")
