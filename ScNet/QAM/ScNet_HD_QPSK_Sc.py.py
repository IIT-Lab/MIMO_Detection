...
def relu_layer(x, input_size, output_size, Layer_num, limit_mat, full_connect):
    y = tf.nn.relu(affine_layer(x, input_size, output_size, Layer_num, limit_mat, full_connect))
    return y


def sign_layer(x, input_size, output_size, Layer_num, limit_mat, full_connect):
    y = affine_layer(x, input_size, output_size, Layer_num, limit_mat, full_connect)
    return y
...
weigth_matrix = np.zeros((3 * (2 * K), 4 * K))
for i in range(0, K):  # 遍历天线
    for j in range(0, 3):  # 遍历Hy、x、HHy
        for k in range(0, 4):  # 遍历16个符号
            weigth_matrix[j * 2 * K + i, i * 4 + k] = 1
            weigth_matrix[j * 2 * K + K + i, i * 4 + k] = 1

weigth_matrix1 = np.zeros((2 * (2 * K), 4 * K))
for i in range(0, 2 * K):  # 遍历天线
    for j in range(0, 2 * K):  # 遍历Hy、x、HHy
        weigth_matrix1[i, j] = 1

for i in range(2 * K, 4 * K):  # 遍历天线
    for j in range(2 * K, 4 * K):  # 遍历Hy、x、HHy
        weigth_matrix1[i, j] = 1
...
for i in range(1, L):
    ...
    ZZ1 = relu_layer(ZZ, hl_size, 4 * K, 'sign' + str(i), weigth_matrix, 1)
    ZZ2 = relu_layer(ZZ1, hl_size, 4 * K, 'sign' + str(i), weigth_matrix, 1)
    ZZ3 = relu_layer(ZZ2, hl_size, 4 * K, 'sign' + str(i), weigth_matrix, 1)
    S2.append(relu_layer(ZZ3, hl_size, 4 * K, 'sign' + str(i), weigth_matrix, 1))
    ...
    BER.append(tf.reduce_mean(tf.cast(
        tf.logical_or(tf.not_equal(x_real, tf.sign(S1[-1][:, 0:K])), tf.not_equal(x_imag, tf.sign(S1[-1][:, K:2 * K]))),
        tf.float32)))

Max_Val = tf.reduce_max(S3, axis=2, keepdims=True)
...
if train_flag:
    ...
    for i in range(train_iter):  # num of train iter
        ...
        if i % 100 == 0:
            TOTAL_LOSS.eval(feed_dict={HY: batch_HY, HH: batch_HH, X: batch_X, X_IND: x_ind})
...
for j in range(num_snr):
    ...
    for jj in range(test_iter):
        ...
        tmp_bers[0, jj] = np.array(
            sess.run(BER[L - 1], {HY: batch_HY, HH: batch_HH, X: batch_X, X_IND: x_ind}))  # /(test_batch_size*K)
        toc = tm.time()
